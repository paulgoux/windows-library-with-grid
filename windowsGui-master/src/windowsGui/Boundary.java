...
public class Boundary{
	  PApplet applet;
	  BMScontrols Bms;
	  public float x1,x2,y1,y2,w,h,x3,y3,size = 1,x4,y4,D1,D2,T1,T2,collision_index,menux,menuy,deltamx,deltamy,tacx,tacy,dir;
	  ...
	  public int key1,key2,type,t,t2,vert,vert1,vert3,trit,id,index,lindex,sindex,ccount,sid,vcount,hcount,uindex = -1,counter
			  ,tpoints,mcount,sides,shapeIndex,lineIndex,lcount,pcount,options = -1,numm,state,state1=1,gid;
	  ...
	  public boolean toggle = false,mdown,mup = true,circle,square,bezier,tri,c_poligon,poligon,drag,none,spline,
			  ...
			  amendInnerBoundary, amendCohesion,amendAvoidance,dashed,debug;
	  ...
	  public Boundary(Window window){
	    ...
	    context = new Menu(0,0,90,contextlabels,Bms);
	    contextDel = new Menu(0,0,90,contextDellabels,Bms);
	    pallete = new Menu(0,200,90,palletelabels,0,Bms);
	    complexsub = new Menu(applet.width-200,applet.height-200,90,complexmenu,Bms);
	    convertToPath = new Menu(0,0,90,convertmenu,Bms);
	    connectedlMenu = new Menu(0,0,90,connectedlMenul,Bms);
	    settings = new sliderBox(0,0,90,5,settingsLabel,false,Bms);
	  ...
	  Boundary(PVector A, PVector B,BMScontrols bms){
	    Bms = bms;
	    applet = bms.applet;
	    p1 = A;
	    p2 = B;
	  ...
	  Boundary(ArrayList<Point> A,BMScontrols bms){
	    Bms = bms;
	    applet = bms.applet;
	  ...
	  Boundary(ArrayList<Point> A,int k,BMScontrols bms){
	    Bms = bms;
	    applet = bms.applet;
	   center = new PVector (0,0);
	    for(int i=0;i<A.size();i++){
	  ...
	  Boundary(Boundary B,BMScontrols bms){
	    Bms = bms;
	    applet = bms.applet;
	  ...
	  void setupPallete(){
	    if(type==-1){
		    palleteTab = new tab(25,applet.height-70,applet.width-50,70,Bms);
		    palleteTab.setvScroll();
		    PApplet.println("boundary setupp ",palleteTab);
//		    if(palleteTab.states.size()>0)palleteTab.states.get(0).scrollable = true;
...
//		    palleteTab.sliderv.h +=20;
	  ...
	  void init(BMScontrols bms) {
		    ...
		    settings = new sliderBox(0,applet.mouseY,90,5,settingsLabel,false,Bms);
		    settings.menu.draggable = false;
		    if(settings.tooltip!=null)settings.tooltip.visible = false;
	  ...
	  public void draw(){
	      ...
	      if((open||none)&&palleteTab!=null)palleteTab.displayTab();
	    ...
	    //for(int i=Boundaries.size()-1;i>-1;i--){
	      ...
	      if(!b.visible&&pos(b,m)){
	        if(visible&&applet.mousePressed&&Bms.mouseButton==PConstants.LEFT)b.visible = true;
	      }
	      if(b.visible){
	      ...
	      if(i==0&&debug)PApplet.println("Boundary draw 01");
	    ...
	    if(clear)clear();
	    if(debug)PApplet.println("Boundary draw 01");
...
//	    if(applet.mousePressed)applet.println("kek");
	  ...
	  void rmenu(){
	    ...
	    if(clear){
	      ...
	      if((open||none)){
	       ...
	       //if(applet.mousePressed&&applet.mouseButton == RIGHT&&!rtoggle){rtoggle = true;}
	         //-----------------------------------------------------------------------------
	  ...
	  void create_circle(){
	    ...
	    Boundary n;
	    if (circle&&!menudown&&applet.mousePressed&&Bms.shapes.toggle==0&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y){
	  ...
	  void create(){
	    ...
	    //temp = new ArrayList<
	      ...
	      if (toggle&&!menudown&&applet.mousePressed&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y){
	  ...
	  void create_tri(){
	    ...
	    if (tri&&applet.mousePressed&&Bms.shapes.toggle==0&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y){
	  ...
	  void create_rect(){
	    ...
	    float ax = applet.mouseX;
	    float ay = applet.mouseY;
	    //if(applet.mousePressed)println(shapes.toggle);
	    if (square&&!menudown&&applet.mousePressed&&Bms.shapes.toggle==0&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y){
	  ...
	  void create_poligon(){
	    ...
	    if (applet.mousePressed&&mcount==0&&!complexsub.pos()&&Bms.shapes.toggle==0&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y){
	      ...
	      updateSliderMenu(n);
	    	...
	    	applet.text("Please define number of sides",100,100);
	  ...
	  void create_complex_poligon(){
	    ...
	    if(complex){
	      ...
	      if(applet.mousePressed&&mcount==0&&!complexsub.pos()&&Bms.shapes.toggle==0&&applet.mouseX>Bms.main.x&&applet.mouseY>Bms.main.y)mdown = true;
	      else mdown = false;
	      if(mdown) mcount++;
	      if(applet.mousePressed&&complexsub.pos()){
	        cfinish = true;
	        temp = new ArrayList<PVector>();
	      }
	      ...
	      if(mdown){
	        ...
	        applet.strokeWeight(20);
	        applet.stroke(0);
	        applet.point(polymidpoint.x,polymidpoint.y);
	        applet.strokeWeight(1);
	        }
	  ...
	  void connectedLines(){
	    ...
	    if(connectedlines){
	      ...
	      if(applet.mousePressed&&mcount==0&&!complexsub.pos()&&Bms.shapes.toggle==0)mdown = true;
	      else mdown = false;
