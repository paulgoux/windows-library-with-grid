...
//    sliderMenu.setClassicSquare();
  ...
  public Grid(float xx, float yy, float ww, float hh,int Res,BMScontrols bms) {
    ...
    grid = applet.createShape(applet.GROUP);
...
//    pallete.toggle(16,this,"saveImage");
  ...
  public void grid_functions() {
    ...
    if (init&&toggle) {
      ...
      seekBoundaries3();
      //applet.text(wallSquares.size(),applet.width-50,100);
      //for(int i=wallSquares.size()-1;i>-1;i--){
      //  Grid_space g = wallSquares.get(i);
      //  applet.fill(255,0,0,150);
      //  //applet.noFill();
      //  if(contour)applet.ellipse(g.x+g.w/2,g.y+g.h/2,g.w,g.h);
      //}
    }
  };
  ...
  public void drawPlain(){
    ...
    for (int yy = 0; yy < rows-1; yy++) {
        ...
        if (terrain3d)applet.beginShape(applet.TRIANGLE_STRIP);
        float xoffr = fx;
  ...
  public void save() {
      ...
      if(debug)applet.println("grid save checklcn ");
      output.open();
      if(debug)applet.println("grid save open ");
      String gridData = x + "," + y + "," + w + "," + h + "," + res + "," 
                        + seed + "," + fy + "," + pxfx + "," + pyfy;
      //String gridData = "";
      applet.println(gridData);
      output.write(gridData);
      if(debug)applet.println("grid save write title ");
      ...
      if(debug)applet.println("grid save writedata ");
      output.close();
      if(debug)applet.println("grid save close ");
      save = true;
      sliderMenu.save.update("terrain","sliders.txt",output.counter);
      if(debug)applet.println("grid save update sldr menu ");
      sliderMenu.save();
      if(debug)applet.println("grid save save slider menu ");
  };

  public void load() {

    if(load &&folder!=null){
      applet.println("load: " + folder);
      //revise
//      String [] temp = fileUtils.listNames(folder);
      ...
      if(temp!=null&&temp.length>=2){
        ...
        for(int i=0;i<temp.length;i++){
          ...
          String [] m = applet.match(temp[i],"grids");
          String [] m1 = applet.match(temp[i],"sliders");
          File f = null;
          if(m!=null){
            //f = new File(folder + temp[i]);
            terrainValues = applet.loadStrings(folder + "\\" + temp[i]);
            applet.println(temp[i]);
          }
          if(m1!=null){
            applet.println(temp[i]);
        ...
        if(terrainValues!=null&&sliderValues!=null){
          ...
          String []init_ = applet.splitTokens(terrainValues[0], ",");
        ...
        if (circular) {
          ...
          for (int i=0; i<360; i+=WW) {
            for (int j=0; j<rows; j++) {
              ...
              x1 = x+w/2 + (r/2+1)*j * applet.cos(i);
              y1 = y+h/2 + (r/2+1)*j * applet.sin(i);
              int num = i + j * cols;
          ...
          if (shape) {
            theta = applet.radians(360/sides);
            ratio = (HH)/2 * applet.sin(theta);
            ratio2 = (HH)/2 * applet.cos(theta);
            ww = ratio *2;
            hh = ratio2*2;
          }
  ...
  public void init2(){
    ...
    for(int i=0;i<rows;i++){
        for(int j=0;j<cols;j++){
          ...
          float height = applet.map(n*amp,0,1,0,255);
          ...
          PShape rect = applet.createShape(applet.RECT,j*res,i*res,res,res);
          rect.setFill(col);
          grid.addChild(rect);
          gridChildren++;
...
//  if(applet.frameCount%10==0)applet.println("grid display",mountainlevel);
    ...
    if (toggle) {
      ...
      for (int i=0;i<rows;i++) {
        for (int j=0;j<cols;j++) {
          ...
          float height1 = applet.map(applet.noise(x1, y1)*amp1, 1, 0, 0, 255);
          float height2 = applet.map(applet.noise(x2, y2)*amp2, 1, 0, 0, 255);
          float height3 = applet.map(applet.noise(x3, y3)*amp3, 1, 0, 0, 255);

          float height = (height1+height2+height3)/3;
          Grid_space a = squares.get(pos);

          float mapHeight = applet.map(height,0,255,0,255);
          ...
          if (mapHeight>mountain) {
            ...
            a.col = applet.color(255, 255, 255, (int) (255-applet.abs(mapHeight/2)*1.2));
            C = applet.color(255, 255, 255, (int) (255-applet.abs(mapHeight/2)*1.2));
          } else if (mapHeight>grass) {
            a.col2 = applet.color(0, 255, 0);
            a.col = applet.color(0, 255, 0, (int) (255-applet.abs(mapHeight/2)*1.1));
            C = applet.color(0, 255, 0, (int) (255-applet.abs(mapHeight/2)*1.1));
          } else if (mapHeight>floor) {
            a.col2 = applet.color(255, 255, 0);
            a.col = applet.color(255, 255, 0, (int) (255-applet.abs(mapHeight/2)*1.1));
            C = applet.color(255, 255, 0, (int) (255-applet.abs(mapHeight/2)*1.1));
          } else if (mapHeight<sea) {
            a.col2 = applet.color(0, 0, 255);
            a.col = applet.color(0, 0, 255, (int) (255-applet.abs(mapHeight/2)*1.1));
            C = applet.color(0, 0, 255, (int) (255-applet.abs(mapHeight/2)*1.1));
          } else {
            a.col2 = applet.color(255, 255, 255);
            a.col = applet.color(255, 255, 255, (int) (255-applet.abs(mapHeight/2)*1.2));
            C = applet.color(255, 255, 255, (int) (255-applet.abs(mapHeight/2)*1.2));
            ...
            if (visible) {
              ...
              applet.fill(a.col);
              if(circle) {
              applet.ellipseMode(applet.CORNER);
  ...
  public void draw_2d() {
    if (terrain3d&&toggle) {
      ...
      applet.rotateX(applet.PI/3+applet.radians(xrot+YY));
      applet.translate(applet.width/2, applet.height/2);
      applet.translate(w/2, h/2);
      applet.rotateZ(applet.radians(zrot+XX));
      applet.translate(-w/2, -h/2);
      applet.translate(w/2, 0);
      applet.rotateY(applet.radians(yrot));
      applet.translate(-w/2, 0);
    ...
    if (toggle) {
      ...
      for (int yy = 0; yy < rows-1; yy++) {
        ...
        for (int xx = 0; xx< cols; xx++) {
          ...
          int pos = xx + yy * cols;
          terrain[xx][yy] = applet.map(applet.noise(xoffr, yoffr), 1, 0, 0, 255);
          perlx[xx][yy] = applet.map(applet.noise(perlx_xoff, perlx_yoff)*map1, 1, 0, 0, 255);
          perly[xx][yy] = applet.map(applet.noise(perly_xoff, perly_yoff)*map2, 1, 0, 0, 255);
          ...
          float R = applet.map((terrain[xx][yy]+num+num1),0,765,0,255);
          ...
          if ((terrain2d||terrain3d)&&!plain) {
            ...
            if (R>mountain) {
              a.col2 = applet.color(255, 255, 255);
              a.col = applet.color(255, 255, 255,  (int) (255-applet.abs(R/2)*1.2));
              C = applet.color(255, 255, 255, (int) (255-applet.abs(R/2)*1.2));
            } else if (R>grass) {
              a.col2 = applet.color(0, 255, 0);
              a.col = applet.color(0, 255, 0, (int) (255-applet.abs(R/2)*1.1));
              C = applet.color(0, 255, 0, (int) (255-applet.abs(R/2)*1.1));
            } else if (R>floor2) {
              a.col2 = applet.color(255, 255, 0);
              a.col = applet.color(255, 255, 0, (int) (255-applet.abs(R/2)*1.1));
              C = applet.color(255, 255, 0, (int) (255-applet.abs(R/2)*1.1));
            } else if (R<sea) {
              a.col2 = applet.color(0, 0, 255);
              a.col = applet.color(0, 0, 255, (int) (255-applet.abs(R/2)*1.1));
              C = applet.color(0, 0, 255, (int) (255-applet.abs(R/2)*1.1));
            } else {
              a.col2 = applet.color(255, 255, 255);
              a.col = applet.color(255, 255, 255, (int) (255-applet.abs(R/2)*1.2));
              C = applet.color(255, 255, 255, (int) (255-applet.abs(R/2)*1.2));
        ...
        if (terrain3d)applet.endShape(applet.CLOSE);
        yoffr += noise1 + zoom;
  ...
  public void debug(Grid_space a) {
    ...
    if (a.pos()&&debug) {
      ...
      for (int i=0; i<a.Walls_b.size(); i++) {
         applet.text(applet.str(a.Walls_b.get(i)) + ", ", X+40+30*i, Y+ vspacing*4);
      }
      ...
      applet.text("Node: " + applet.str(a.node), X,Y + vspacing * 7);
      applet.text("Conn. Neighbours: " + a.connections.size(),X,Y + vspacing * 8);
      applet.text("Toggle: " + applet.str(a.toggle),X,Y + vspacing * 9);
      applet.text("Pop Visited: " + applet.str(a.popVisited),X,Y + vspacing * 10);
      applet.text("Visited: " + applet.str(a.visited),X,Y + vspacing * 11);
      applet.text("Path Connected: " + applet.str(a.pathConnected),X,Y + vspacing * 12);
  ...
  public void popAgents() {
    if (populateBoundaries&&!agentsPopulated) {
      for (int i=0; i<totalAgents; i++) {
        ...
        Grid_space g = squares.get(applet.floor(applet.random(squares.size())));
        g.popId = i;
        g.pid = i;
  ...
  public void gshape(int sides, Grid_space a, int col) {
    ...
    for (int i=0; i<sides; i++) {
      if (a.Walls_a.get(i)==true) {
        ...
        applet.strokeWeight(1);
        applet.line(a.x+a.w/2*applet.sin(thetab), a.y+a.w/2*applet.cos(thetab), a.x+a.w/2*applet.sin(thetac), a.y+a.w/2*applet.cos(thetac));
  ...
  public void keys_() {
    ...
    if (applet.keyPressed && applet.keyCode ==37) {
      if (!terrain3d){
        fx += noise1;
        pxfx += applet.pow(noise1,map1);
        pyfy += applet.pow(noise2,map2);
      }else {
        fy += noise1;
        pxfx += applet.pow(noise1,map1);
        pyfy += applet.pow(noise2,map2);
...
class Shape {
  ...
  Shape(int Sides, Grid_space Parent) {
    ...
    for (int i=0; i<sides; i++) {
      float thetab =  theta * i;
      float thetac =  theta * i+theta;
      PVector a = new PVector (x+w/2*applet.sin(thetab), y+w/2*applet.cos(thetab));
      PVector b = new PVector (x+w/2*applet.sin(thetac), y+w/2*applet.cos(thetac));
