...
class Entity{
	...
	int id,xp,yp,txtb_value,vcount,polarity = PApplet.floor(applet.random(-2,2)),pos,race,ent,function = PApplet.floor(applet.random(0,6)),eIndex1 = -1,eIndex2 = -1,cIndex1,cIndex2,aIndex1,aIndex2,hRaces,acount,scount,alcount,mcount,gid,xpos,ypos,range;
	
	public float x,y,z,w,h,intensity,mass = applet.random(1,2),radius = applet.random(10,30), lifespan,intelligence,dexterity,endurance,piety,vision,health,speed,selfp,charisma,agility,reputation,sanity,humanity,faith,fresistance;
	
	public float homex,homey,homew,homeh,birthrate = 2000;
	public float memory,movement,respiration,sensitivity,growth,regulation,nutrition,synthesis,transportation,reparation,
			     size,tscale = 0.3f,ssize = 1,hunger,h_cooldown,i_cooldown;
	
	public float age = 0,bscale = 0.2f,time,delay,r,skill,vlimit,vlimitb = vlimit,fov = 20,
			eating_time = applet.random(10,20),breathingroom,breathingroomb,breathingroomb1 = breathingroom,
			breathingroomb2 = breathingroomb,alimit,mult1,mult2,mu,min,minb;
	
	float  bdir = 0,inc = 0,countdown = applet.random(10),countdownb = countdown,offset = 100,
			valueOffset = applet.random(-offset/2,offset/2),Friction,maxSeek,maxAvoid,maxAlign,minSeek,
			minAvoid,minAlign,maxRange,minRange;
	
	public float avoidance = applet.random(0,5),cohesion = applet.random(0,5),cohesionb,alignment,neg = 0,
			wandertheta = 0,wanderm = 0.2f,wandermb = wanderm,cohesionb1 = cohesion,cohesionb2 = cohesionb,
			separationc=1,cohesionc=1,separation = applet.random(0,15),separationb = applet.random(0,15),
			alimitb = alimit, alignmentb = alignment,avoidanceb = avoidance,alignmentc = 1,separationd = 20,separationdb
	...
	float vx,vy,vz,ax = 0.02f,ay = 0.02f,az = 0.1f,dir = applet.random(-2*PConstants.PI,2*PConstants.PI),sumvx,sumvy,tgdist;
	public String label;
	...
	int col ;
	int col2 = col,cols,colb = col,hcol,col3;
	...
	Float [] avoid_angles = {applet.radians(0),applet.radians(45),applet.radians(-45),applet.radians(90),applet.radians(-90),applet.radians(135),applet.radians(-135),applet.radians(180)};
	Float [] avoid_angles2 = {applet.radians(90),applet.radians(-90),applet.radians(135),applet.radians(-135),applet.radians(180)};
	...
	Entity(float xx, float yy,float ww,float hh,BMScontrols bms){
	  Bms = bms;
	  applet = bms.applet;
	  homex = xx;
	  homey = yy;
	...
	ArrayList<Human> getNeighbours2(){
	  ...
	  float max = PApplet.round(e.Humans.get(race).get(0).maxRange);
	  float min = PApplet.floor(e.Humans.get(race).get(0).minRange);
	  //int clip = 0;
	  //if(k1>0)clip = k1;
...
//	    reset = false;
	...
	public void separationLogic(){
	  ...
	  for(int i=0;i< e.Humans.size();i++){
	    ...
	    float xx = applet.width/22;
	    float yy = applet.height/2;
	    float theta = PApplet.radians(360/e.Humans.size());
	    float r = 200;
	
	    float x1 = xx + r * PApplet.cos(theta*i);
	    float y1 = yy + r * PApplet.sin(theta*i);
	
	    applet.fill(e.Humans.get(i).get(0).col);
...
//		  if(applet.mousePressed){
	      ...
	      eIndex2 = -1;
	        ...
	        e2down = true;
	        mdown = true;
	        e.Humans.get(eIndex1).get(0).avoidNeighbour.set(eIndex2,applet.random(r*2,50));
	        }}else {
...
//	        	  if(applet.mousePressed&&!mdown&&d1<15){
	            ...
	            for(int i=0;i< e.Humans.size();i++){
	              ...
	              if(j!=i)e.Humans.get(i).get(0).avoidNeighbour.set(j,applet.random(r*2,50));
...
//		  if(applet.mousePressed){
	      ...
	      cIndex2 = -1;
	        ...
	        e2down = true;
	        mdown = true;
	        e.Humans.get(cIndex1).get(0).seekNeighbour.set(cIndex2,applet.random(5,40));
	        if(cIndex1==cIndex2)e.Humans.get(cIndex1).get(0).seekNeighbour.set(cIndex2,applet.random(r*2,50));
	        }}else {
	          if(applet.mousePressed&&Bms.mouseButton==PConstants.LEFT&&!mdown&&d1<15){
//	        	  if(applet.mousePressed&&!mdown&&d1<15){
	            mdown = true;
	            for(int i=0;i< e.Humans.size();i++){
	              ...
	              if(j!=i)e.Humans.get(i).get(0).seekNeighbour.set(j,applet.random(r*2,50));
...
//	  if(applet.mousePressed){
	      ...
	      aIndex2 = -1;
	        ...
	        e2down = true;
	        mdown = true;
	          e.Humans.get(aIndex1).get(0).alignNeighbour.set(aIndex2,applet.random(r*2,50));
	        }}else {
	          if(applet.mousePressed&&Bms.mouseButton==PConstants.LEFT&&!mdown&&d1<15){
//	        	if(applet.mousePressed&&!mdown&&d1<15){
	            mdown = true;
	            for(int i=0;i< e.Humans.size();i++){
	              ...
	              if(j!=i)e.Humans.get(i).get(0).alignNeighbour.set(j,applet.random(r*2,50));
...
//	          	else if(applet.mousePressed&&!mdown&&d1<15){
	...
	public void runh(){
	  ...
	  if(toggle){
	    ...
	    if(!EntitiesSublist.pos())new_human(m);
	  ...
	  for ( int i=0;i<e.Humans.size();i++) {
	    ...
	    for(int j=H.size()-1;j>-1;j--){
	     ...
	     if(h.conception){
	       ...
	       Human entity = new Human(h.x + applet.random(-20,20),h.y + applet.random(-20,20),Humans.get(h.race).size(),h.race,h.ent);
	       entity.parent = h;
	...
	public void create(int n,int type){
	  ...
	  for(int i=0;i<n;i++){
	    ...
	    if(type==0){
	      Bms.entities.get(ent).Humans.get(race).add(new Human(x,y,k,species.id,ent));
	    }else if(type==1){
