...
public class TextBox {
	  BMScontrols Bms;
	  PApplet applet;
	  int id, toggle, cols, rows, size, t, timer = 20, blkrate = 30, t1 = blkrate, t2 = blkrate, start, end, hcount, index,
			  lindex, vindex = -1, hindex = -1, windex, vpos = 0, hpos, sltcounter, vcount = 0,delay=10,delay2 = 5,maxCount = 51;
	  public float x, y, w, h, bx,by,bw,bh, textsize = 10, twidth = 0, posx, posy, tposx, tposy, strposx, strposy, tbwidth,
			  strwidth, strwidth2, strheight, cursorx, cursory, crwidth, totalwidth,xoff,xOffset,yOffset,r1,r2,r3,r4;
	  String label, text, label_backup, cboard = "";
	  public boolean drag, resize, hover, border, background, hidden, fill = true, init, ready, label_bool, clear, copied, tbox = false, full, tsize,dragtext,mdown,
	          onfocus,showLabel,parentCanvas,firstPress,getChar,selectAll,ctrl,copyClipboard,clipboardToggle,debug;
	  public Menu toolBox;
	  public Button child;
...
//		if(applet.mousePressed)applet.println("textbox draw");
	    ...
	    getKey();
	    if(debug)applet.println("tbox draw start");
	    if (textBox!=""&&applet.key== PConstants.ENTER)value = Float.parseFloat(textBox);
	    //debug();
	    if(showLabel&&label!=null){
	      ...
	      applet.textSize(12);
	      if(mouse!=null&&applet.mousePressed&&pos()||toggle==1)showLabel = false;
...
//	    if(applet.frameCount%10==0)applet.println("textbox drawend");
	  ...
	  public void selectall() {
	    ...
	    if (toggle==1&&ctrl&&selectAll)applet.text("Select all", 100, 200);
	  ...
	  public void getKey(PGraphics canvas) {
	    ...
	    if (pos(mouse)||toggle>0) {
	      ...
	      if (!copyClipboard&&!clipboardToggle) {
	        ...
	        if (a!=null) l = new Letter(a, x+strwidth, y + vcount * textsize,Bms);
	        if (a!=null&&getChar&&!exclusion()&&applet.key!=PConstants.BACKSPACE&&applet.key!=PConstants.ENTER) {
	        ...
	        } else if (applet.keyPressed && applet.key==PConstants.BACKSPACE&&getChar) {
	          delete();
	        } else if (applet.keyPressed && applet.keyCode==PConstants.ENTER&&getChar) {
...
//	    if(applet.frameCount%10==0)applet.println("textbox getkey");
	  ...
	  public void conditionals(PGraphics canvas) {
	    ...
	    if (applet.mousePressed&&(pos(mouse)))toggle=1;mdown = true;
	...
	public void constants() {
	  ...
	  boolean exclusion() {
	    ...
	    if (applet.keyCode==37||applet.keyCode==38||applet.keyCode==39||applet.keyCode==40||applet.keyCode==33||applet.keyCode==34||applet.keyCode==17||applet.keyCode==18||applet.keyCode==9) {
	      k = true;
	    } else if (applet.keyCode==20||applet.keyCode==16||applet.keyCode==255||applet.keyCode==91||applet.keyCode==36||applet.keyCode==35||applet.keyCode==45||applet.keyCode==46||applet.keyCode==8) {
	      k = true;
	    }
	    return k;
	  };
...
//	    if(applet.frameCount%10==0)applet.println("textbox mpos");
	  ...
	  public void getClick() {
	    if (pos() && applet.mousePressed)toggle++;
	  ...
	  public void getClick(PVector m) {
	    ...
	    if (pos(m) && applet.mousePressed)toggle++;
	  ...
	  public void nav() {
	    ...
	    if (textbox.size()>0) {
	      ...
	      if (hindex<=textbox.size()-1&&hindex>-1) {
	        if (applet.keyPressed && applet.keyCode == 39 &&getChar) { 
	          hindex ++;
	        }
	      }
	      //if(applet.keyPressed && applet.key == UP && strheight >0) hindex += vpos *2;
	      //if(applet.keyPressed && applet.key == DOWN && strheight )

	      if (hindex==-1) {
	        if (applet.keyPressed && applet.keyCode == 37 &&getChar) {
